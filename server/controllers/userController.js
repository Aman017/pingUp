...
if(tempUser.username !== username){
    ...
    if(user){
        // we will not change the user name if it is alredy taken
...
//Send connection request
 ...
 export const sendConnectionRequest = async (req, res)=>{
        try {
            const {userId} = req.auth();
            const {id} = req.body
            //check if user has send more than 20 connection requets in the last 24 hours
            ...
            if(connectionRequests.length >= 20){
                return res.json({success:false, message:'You have sent more than 20 connection request in the last 24h'})
            }
            
            //check if user alerdy connected
