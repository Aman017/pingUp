  ...
  return (
    <div className='w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4'>
        ...
        <div className='flex gap-4 pb-5 '>
            {/* add stories card */}
             <div onClick={()=>setShowModal(true)} className=' rounded-lg shadow-sm min-w-[7.5rem] max-w-[7.5rem] max-h-40 aspect-[3/4] cursor-pointer hover:shadow-lg transition-all duration-200 border-2 border-dashed border-indigo-300 bg-gradient-to-b from-indigo-50 to-white '>
...
<p className='text-sm font-medium text-slate-700 text-center'>Create Story</p>
             ...
             stories.map((story, index)=>(
                <div onClick={()=> setViewStory(story)} key={index} className={`relative rounded-lg shadow min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-gradient-to-b from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95`}>
                    ...
                    <p className='text-white absolute bottom-1 right-2 z-10 text-sm'>{moment(story.createdAt).fromNow()}</p>
                    {
